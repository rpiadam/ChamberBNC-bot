...
class Mail
  ...
  def self.ticket_created(to_addr, ticket)
    msgstr = <<-END_OF_MESSAGE
      From: ChamberBNC <bot@chamberirc.net>
      To: #{to_addr}
      Reply-to: admin@chamberirc.net
      ...
      Creator: #{ticket.creator} on #{ticket.network}
      Created: #{ticket.created_time.strftime('%Y-%m-%d %H:%M:%S UTC')}

      Message:
      #{ticket.message}

      View and manage tickets in the admin channel or use:
  ...
  def self.ticket_updated(to_addr, ticket, replier, message)
    msgstr = <<-END_OF_MESSAGE
      From: ChamberBNC <bot@chamberirc.net>
      To: #{to_addr}
      Reply-to: admin@chamberirc.net
      ...
      Support ticket ##{ticket.id} has been updated:

      Ticket: ##{ticket.id} - #{ticket.subject}
      Status: #{ticket.status}
      Assigned: #{ticket.assigned_to || 'Unassigned'}

      New reply from #{replier}:
      #{message}

      Total replies: #{ticket.replies.size}

      View ticket: !ticketinfo #{ticket.id}
